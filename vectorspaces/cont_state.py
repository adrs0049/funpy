...
class ContinuationState(BaseState):
    ...
    @classmethod
    def from_coeffs(cls, coeffs, pname, n_funs, *args, **kwargs):
        assert (coeffs.size - 1) % n_funs == 0, 'coeffs does not have correct size!'
        soln = Fun.from_coeffs(coeffs[:-1], n_funs, *args, **kwargs)
        ap = Parameter(**{pname: np.real(coeffs[-1].item())})
        return cls(a=ap, u=np.real(soln))

    @classmethod
    def from_fun(cls, fun, pname, pvalue, *args, **kwargs):
        ap = Parameter(**{pname: float(pvalue)})
        return cls(a=ap, u=np.real(fun))

    @classmethod
    def from_state(cls, other, pname, pvalue, *args, **kwargs):
        ap = Parameter(**{pname: float(pvalue)})
        pars = {n: v for n, v in other.items()}
        return cls(a=ap, u=other.u, *args, **pars, **kwargs)
    ...
    def flatten(self):
        return np.hstack((self.funcs[0].flatten(), self.reals[0].value))
