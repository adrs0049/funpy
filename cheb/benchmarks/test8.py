...
if __name__ == '__main__':
    ...
    functions = clenshaw, eval_lib.clenshaw_vector
    times = {f.__name__: [] for f in functions}

    N = 101
    k = 100

    fun = Fun(op=[lambda x: np.cos(k * np.pi * x / 2), lambda x: np.sin(k * np.pi * x / 2)], simplify=False)
    xs = np.linspace(-1, 1, 1001)

    tol = 1e-10
    for _ in range(N):
        fun = Fun(op=[lambda x: np.cos(k * np.pi * x / 2), lambda x: np.sin(k * np.pi * x / 2)])
        fun.prolong(1001)
        ...
        t0 = time.time()
        r2 = eval_lib.clenshaw_vector(xs, coeffs)
        t1 = time.time()
        times[eval_lib.clenshaw_vector.__name__].append((t1 - t0) * 1000)
